...
def update_mean(
    ...
    current_mean = current_mean + \
        torch.sum((weight / current_weight_sum) * (value - current_mean))
...
def stable_mean_dist_reduce_fn(state: torch.Tensor) -> torch.Tensor:
    ...
    mean, weight_sum = update_mean(
        current_mean=torch.as_tensor(
            0.0, dtype=state.dtype, device=state.device),
        current_weight_sum=torch.as_tensor(
            0.0, dtype=state.dtype, device=state.device),
        value=state[:, 0],
        weight=state[:, 1],
    )
    ...
    def update(self, value: torch.Tensor, weight: Union[float, torch.Tensor] = 1.0) -> None:
        ...
        if not isinstance(weight, torch.Tensor):
            weight = torch.as_tensor(
                weight, dtype=value.dtype, device=value.device)
